...
void setup()
{
  ...
  myservo3.attach(11);  // attaches the servo on pin 10 to the servo object
  myservo4.attach(6);  // attaches the servo on pin 10 to the servo object
...
void loop()
{
  ...
  if ((myICM.status == ICM_20948_Stat_Ok) || (myICM.status == ICM_20948_Stat_FIFOMoreDataAvail)) // Was valid data available?
  {
    ...
    if ((data.header & DMP_header_bitmap_Quat6) > 0) // Check for GRV data (Quat6)
    {
      ...
      */
      double heading = atan2((2 *(q0 * q3 + q2 * q1)), (1 - (2 * ((q2 * q2) + (q3 * q3)))));      //ψ Heading {\displaystyle \psi } \psi : rotation about the Z-axis
      double pitch = asin(2 * (q0 * q2 - q3 * q1));                                               //θ Pitch {\displaystyle \theta } \theta : rotation about the new Y-axis
      double bank = atan2((2 *(q0 * q1 + q2 * q3)), (1 - (2 * ((q1 * q1) + (q2 * q2)))));         //ϕ Bank {\displaystyle \phi } \phi : rotation about the new X-axis
                                                                                                  //More info on https://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles
      x1 = (heading * 4068.0) / 71.0;
      y1 = (pitch  * 4068.0) / 71.0;
      z1 = (bank  * 4068.0) / 71.0;
      ...
      myservo1.write(x1);
      myservo2.write(y1 + 90);
      myservo3.write(z1);

      
      // read the input on analog pin 0:
      int sensorValue = analogRead(A0);
      double degreeGuess = map(sensorValue, 140, 840, 0, 180);
      x2 = degreeGuess;
      
      myservo4.write(180 - x2);

      SERIAL_PORT.print(x1, 3);
      SERIAL_PORT.print("        ");
      SERIAL_PORT.print(y1, 3);
      SERIAL_PORT.print("        ");
      SERIAL_PORT.print(z1, 3);
      SERIAL_PORT.print("        ");
      SERIAL_PORT.print(x2, 3);
      SERIAL_PORT.println("        ");
